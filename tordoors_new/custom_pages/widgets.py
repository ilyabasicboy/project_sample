...
from tordoors_new.custom_catalog.models import Product, Facing
...
class PageAdvantageSelect(Select):

    def __init__(self, language=None, attrs=None, **kwargs):
        super().__init__(attrs)
        print(attrs)
        groups = Page.objects.filter(template='pages/advantages.html')
        choices = [('', u'-----------')] + [(group.id, group.title()) for group in groups]
        self.choices = list(choices)
...
class ProductsMultiSelect(FilteredSelectMultiple):

    def __init__(self, language=None, attrs=None, **kwargs):
        super(ProductsMultiSelect, self).__init__(language, attrs, **kwargs)
        products = Product.objects.filter(show=True)
        self.choices = list(((product.id, product.title) for product in products))

    def render(self, name, value, attrs=None, **kwargs):
        self.verbose_name = u'Товары'
        self.is_stacked = False
        if value:
            value = value[1:len(value) - 1].split(', ')
            value = [int(id.strip()) for id in value]
        rendered = super(ProductsMultiSelect, self).render(name, value, attrs, **kwargs)
        return rendered
    ...
    def render_options(self, choices, selected_choices):
        if isinstance(selected_choices, str):
            selected_choices = selected_choices[1:-1]
            selected_choices = [s[2:-1] for s in selected_choices.split(', ')]
        return super(ProductsMultiSelect, self).render_options(choices, selected_choices)

    def value_from_datadict(self, data, files, name):
        return [int(str_id) for str_id in data.getlist(name)]


class FacingSelect(Select):

    def __init__(self, language=None, attrs=None, **kwargs):
        super().__init__(attrs)
        groups = Facing.objects.exclude(facing_colors=None)
        choices = [('', u'-----------')] + [(group.id, group.name) for group in groups]
        self.choices = list(choices)


register_widget(Checkbox)
register_widget(GallerySelect)
register_widget(PageCoopSelect)
register_widget(PageAdvantageSelect)
register_widget(MultiSelect)
register_widget(ProductsMultiSelect)
register_widget(FacingSelect)
