...
from tordoors_new.custom_catalog.models import Product
from tordoors_new.custom_pages.models import Logo, Advantage
...
def show_block_cards(context, num_pagination=12):
    ...
    context['num_pagination'] = num_pagination
    return context
...
def get_logos(page):
    ...
    """ Возвращает список логотипов на текущей странице """

    page = Page.objects.filter(id=page).first()

    return Logo.objects.filter(
...
@register.simple_tag()
def get_advantages(page):

    """ Возвращает список логотипов на текущей странице """

    return Advantage.objects.filter(
